...
type (
	ConsumeHandle func(key string, value []byte) error
...
func MustNewQueue(c Config, handler ConsumeHandler, opts ...QueueOption) queue.MessageQueue {
...
func NewQueue(c Config, handler ConsumeHandler, opts ...QueueOption) (queue.MessageQueue, error) {
...
func newKafkaQueue(c Config, handler ConsumeHandler, options queueOptions) queue.MessageQueue {
	var offset int64
...
func (q *kafkaQueue) consumeOne(key string, val []byte) error {
	startTime := timex.Now()
	...
	for i := 0; i < q.c.Processors; i++ {
		q.consumerRoutines.Run(func() {
			for msg := range q.channel {
				if err := q.consumeOne(string(msg.Key), msg.Value); err != nil {
...
func ensureQueueOptions(c Config, options *queueOptions) {
	...
	if options.metrics == nil {
		options.metrics = stat.NewMetrics("iotkq")
	}
}
